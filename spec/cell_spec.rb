...
describe TerminalTable::Table do
  Cell = TerminalTable::Table::Cell
  
  it "should default alignment to the left" do
    cell = Cell.new :value => 'foo', :table => TerminalTable::Table.new, :index => 0
    cell.value.should == 'foo'
    cell.alignment.should == :left
  end

  it "should allow overriding of alignment" do
    cell = Cell.new :value => 'foo', :alignment => :center, :table => TerminalTable::Table.new, :index => 0
    cell.value.should == 'foo'
  ...
  it "should allow multiline content" do
    cell = Cell.new :value => "barrissimo\n"+"foo".yellow, :table => TerminalTable::Table.new, :index => 0
  ...
  it "should allow colorized content" do
    cell = Cell.new :value => "foo".red, :table => TerminalTable::Table.new, :index => 0
  ...
  it "should render padding properly" do
    @table = TerminalTable::Table.new(:rows => [['foo', '2'], ['3', '4']], :style => {:padding_right => 3})
